		...
		develop_content: (
			<React.Fragment>
				...
				<ul>
					...
					<li>
						펀딩 생성 등 상호작용이 필요한 부분은 'use client'
				...
				<br />
				<img src="../cinemoa_create.gif" alt="펀딩 생성" width={600} />
...
function Project3() {
	return {
		...
		review: (
			<React.Fragment>
				<h3>React Query를 활용한 서버 상태 관리</h3>
				<ul>
					<li>
						<b>이슈 :</b> 동일한 검색어로 검색할 때, 예약 가능 시간
						조회 등 매번 로딩 상태와 에러 상태를 수동으로 관리해야
						했고, 동일한 데이터를 여러 컴포넌트에서 중복 요청하는
						문제가 발생했습니다. 그래서 불필요한 네트워크 요청이
						빈번하게 발생하는 문제도 있었습니다.
					</li>
					<li>
						<b>해결 :</b> React Query를 도입하여 서버 상태 관리를
						체계화했습니다. useQuery 훅을 활용하여 API 호출 로직을
						선언적으로 작성하고, 자동으로 제공되는 로딩, 에러, 성공
						상태를 활용했습니다.
					</li>
					<li>
						<b>성과 :</b> 자동 캐싱으로 불필요한 네트워크 요청이 약
						60% 감소했습니다. 또한 DevTools를 통해 API 응답 상태를
						시각적으로 모니터링할 수 있어 디버깅 효율성이 크게
						향상되었습니다.
					</li>
				</ul>
				...
				<ul>
					<li>
						...
						상황에서, 사용자가 "24시간"을 검색할 경우 5글자 입력 시
						총 5번의 API 호출이 발생하고 각각 수 천개의 데이터를
						조회하여 서버 부하 증가 및 불필요한 네트워크 트래픽 발생
					</li>
					<li>
				...
				<p>
					...
					지속적으로 수행하여 팀원들과 상호 신뢰를 쌓고 프로젝트가
					문제없이 진행되도록 노력했습니다. 브랜치에 Merge하기 전 MR을
					올려서 전체 BE 팀원들의 코드 리뷰를 받아야 했기 때문에 그
					과정에서 퀄리티 있고 깔끔한 코드 작성법에 대해서 배울 수
					있었습니다.
				</p>
			</React.Fragment>
		),
...
function Project4() {
	return {
		...
		develop_content: (
			<React.Fragment>
				<h3>Pinia를 활용한 상태 관리</h3>
				<ul>
					<li>
						<b>이슈 :</b> 사용자 인증 정보, 챌린지 진행 상태 등 여러
						컴포넌트에서 공유해야 하는 데이터들이 증가하면서 Props
						drilling 문제가 발생했습니다. 또한 컴포넌트 간 데이터
						동기화를 위해 복잡한 props 체인을 사용해야 했고, API
						호출 로직이 각 컴포넌트에 분산되어 중복 코드가
						발생했습니다.
					</li>
					<li>
						<b>해결 :</b> Pinia를 도입하여 도메인별로 Store를
						분리했습니다. 사용자 인증 정보, 챌린지 진행 상태 등
						기능별로 독립적인 Store를 생성하고, TypeScript
						인터페이스를 정의하여 타입 안전성을 보장했습니다. API
						호출 로직을 Store의 actions에 중앙화하여 컴포넌트에서는
						단순히 Store의 actions를 호출하도록 구현했습니다. 또한
						getters를 활용하여 계산된 상태나 필터링된 데이터를
						제공하여 컴포넌트의 로직을 단순화했습니다.
					</li>
					<li>
						<b>성과 :</b> Props drilling 문제가 해결되어 컴포넌트 간
						데이터 전달이 간소화되었고, 상태 관리 로직의 중앙화로
						코드 재사용성이 향상되었습니다.
					</li>
				</ul>
